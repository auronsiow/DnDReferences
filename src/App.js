...
class App extends Component {
  ...
  render() {
    ...
    this.state.loaded && console.log(JSON.stringify(this.state))
    ...
    _.forOwn(this.state.classType, function(value, key) { 
      ...
      liList.push(
        <Jumbotron id={'jumbo_'+value.name}>
          ...
          <AbilityComponent headerText={'Abilities: '} buttonText={value.savingThrows} triggerText={self.state.abilities} />
    ...
    const menuItemList = _.map(this.state.classType, function(c, index) {
      ...
      return (<MenuItem onSelect={self.scrollToClassType.bind(this, c.name)} eventKey={eventKeyIndex}>{c.name}</MenuItem>)
    });
